...
clc
%% load all
[filename,filepath] = uigetfile('*.mat','Select a .mat file to process',...
...
for nfile = 1:length(filename)
    ...
    for nplot = 1:3
        ...
        plot(x,TC_0beg_avg_mat(nplot,:)',color=RegCHXcolor(ntmp,:),...
            DisplayName=['TC ' convertStringsToChars(AxisLabel(nplot)) ' (' BigData(nfile).Conf.Parameters.Orientation ')'])
    end
    legend(NumColumns=2)
end
